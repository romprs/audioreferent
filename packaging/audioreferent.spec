...
%build
# Заглушка: реальная сборка/установка зависимостей происходит в %install,
# venv нужно создавать сразу по конечному пути (/opt/%{name}/venv) — если
# создать его здесь и потом скопировать, у pip-сгенерированных shebang'ов
# скриптов и pyvenv.cfg останется путь сборочной директории, а не
# установочный (тот же приём, что и в пакете redmail).
